...
void professorMenu() {
    ...
    do {
        ...
        if (choice == 1) {   // Assign grade to student.
            ...
            } while (courseChoice > prof.getTaughtCourses().size() || courseChoice < 0);   // Makes sure the courses stay in bounds.

            // Prints to verify the course you selected.
            cout << "Course ";
            prof.getTaughtCourses()[courseChoice - 1]->displayCourseDetails();
            cout << " selected.\n";
            courseCode = prof.getTaughtCourses()[courseChoice - 1]->courseCode;   // Gets the course code.

            // Select a student in the course you teach.
            do {
                cout << "Students in the course\n";
                // If there are no students enrolled in the course.
                if (prof.getTaughtCourses()[courseChoice - 1]->enrolledStudents.size() == 0) {
                    cout << "No students in this class.\n";
                    break;
                }
                else {
                    for (int i = 0; i < prof.getTaughtCourses()[courseChoice - 1]->enrolledStudents.size(); i++) {
                        cout << i + 1 << ". ";
                        prof.getTaughtCourses()[courseChoice - 1]->enrolledStudents[i]->viewProfile();
                    }
                    cout << "Select student (list number): ";
                    cin >> studentChoice;
                }

            } while (studentChoice > prof.getTaughtCourses()[courseChoice - 1]->enrolledStudents.size() || studentChoice < 0);
            // Adds student if there IS a student to select.
            Grade<double> newGrade;
            if (prof.getTaughtCourses()[courseChoice - 1]->enrolledStudents.size() > 0) {
                // May change later.
                student = *prof.getTaughtCourses()[courseChoice - 1]->enrolledStudents[studentChoice - 1];

                // 
                prof.assignGrade(prof.getTaughtCourses()[courseChoice - 1]->enrolledStudents[studentChoice - 1], courseCode, new Grade<double>);
            }
        }
